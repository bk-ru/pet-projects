...
def my_softmax(values: np.ndarray, T=1.):
    ...
    """
    values_ = values / T
    values_normal = values_ - np.max(values)
    values_exp = np.exp(values_normal)
    probas = values_exp / sum(values_exp)
...
class QLearningAgent:
    ...
    def update(self, state, action, reward, next_state):
        ...
        qvalue = (1 -learning_rate) * old_qvalue + learning_rate * (reward + gamma * V_next)
...
class EVSarsaAgent(QLearningAgent):
    ...
    def get_value(self, state):
        ...
        value = np.sum(probabilities[a] * self.get_qvalue(state, a) for a in possible_actions)
        assert value is not None
